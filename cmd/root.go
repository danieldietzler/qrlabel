...
func init() {
	...
	rootCmd.Flags().StringVarP(
		...
		"The file containing the label values. If not provided, the input will be read from stdin."+
			"Each line represents a new label. The use of a separator is optional. If there is none, label and content will be the same. A %n in the label text will enforce a line break (escape with \\%n)",
	)
	rootCmd.Flags().StringVarP(
